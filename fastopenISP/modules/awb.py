...
class AWB(BasicModule):
    ...
    def execute(self, data):
        ...
        if 'RGB' in data:
            self.r_gain = np.array(data['RGB'][0], dtype=np.uint32)  # x1024
            self.gr_gain = np.array(data['RGB'][1], dtype=np.uint32)  # x1024
            self.gb_gain = np.array(data['RGB'][1], dtype=np.uint32)  # x1024
            self.b_gain = np.array(data['RGB'][2], dtype=np.uint32)  # x1024
