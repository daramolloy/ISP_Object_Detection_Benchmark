...
from fastopenISP.modules.basic_module import MODULE_DEPENDENCIES
...
class Pipeline:
    ...
    def execute(self, data, save_intermediates=False, verbose=True):
        """
    ...
    def run(self, raw_path, save_dir, load_raw_fn, suffix=''):
        ...
        data = load_raw_fn(raw_path)
        data, _ = self.execute(data, save_intermediates=False, verbose=False)
        output = cv2.cvtColor(data['output'], cv2.COLOR_RGB2BGR)
        filename = op.splitext(op.basename(raw_path))[0]
        cv2.imwrite(f"{save_dir}{filename}.png", output)
        del data
        del output
        # except Exception as e:
        #     print(f"ISP Failed on Image: {raw_path} {e}")
    ...
    def batch_run(self, raw_paths, save_dirs, load_raw_fn, suffixes='', num_processes=1):
        ...
        savedFiles = 0
        pbar = tqdm(total=num_files, desc="Applying ISP")

        ## Continue when the number of images given equals the number of images saved
        while(savedFiles != num_files):
            numSavedToFolder = len(glob.glob(save_dirs+"*.png"))
            numCurrProcesses = savedFiles - numSavedToFolder

            if numCurrProcesses < num_processes:
                p = mp.Process(target=self.run,
                           kwargs={'raw_path': raw_paths[savedFiles],
                                   'save_dir': save_dirs,
